...
/*
	M is Prime , So M is Odd , So M - 1 is even and (M - 1) / 2 is Integer
	
	    Y ^ 2 = N % M
	=>  Y ^ (M - 1) = N ^ ((M - 1) / 2)
	
	=>  (Y ^ (M - 1)) % M = (N ^ ((M - 1) / 2)) % M  ... Mod in Both Side
	
	=>  (N ^ ((M - 1) / 2)) % M  = 1  ... as  (Y ^ (M - 1)) % M = 1 ... From Fermat's Theorem
	
	So if Y ^ 2 = N % M , has a Solution then ----- (N ^ ((M - 1) / 2)) % M  is 1  .. Euler's Crietion
